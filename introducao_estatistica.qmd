...
pacman::p_load(tidyverse, here, readxl, janitor, dplyr, magrittr, psych, gt, kableExtra, glue, Rlab, tibble)
```
...
7. (Fonte Fávero e Belfiore, 2017 Cap. 8) Um grupo de 60 leitores fez uma avaliação de três livros de romance
e, ao final, escolheram uma das três opções. Teste a hipótese nula de que não há diferença na preferência dos
leitores, ao nível de significância de 5% (*p < 0.05*). Os dados estão disponíveis na planilha Lista de Exercício Complementares: aba Exercício 7.
...
Construindo a função para a estatística qui-quadrado ($X^{2}$)
